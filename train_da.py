...
from DiverseDataSampler.dautils import ModelUtils
from DiverseDataSampler.dapicker import DaTechniques
...
def cli_main():
    ...
    parser.add_argument("--cycles", default=5, type=int, help="Number of Cycles to run Diversity Algorithm")
    parser.add_argument("--DIVERSE_DATA_PATH", type=str, help="Path to prepare and store Diverse Dataset")
    parser.add_argument("--subset_size", default=0.5, type=float, help="Subset of total data to be picked by Diversity Algorithm")
    parser.add_argument("--DA_TECHNIQUE", default="DA_STD", type=str, help="Type of DA Technique(DA_STD or DA_FAST)")
    parser.add_argument("--sample_size", default=None, type=float, help="Subset of dataset to be picked randomly by Faster Diversity Algorithm")
    parser.add_argument("--embedding_size", type=int, help="Size of model's output embedding Required when passing model")
    ...
    train_image_paths = list(paths.list_images(args.DATA_PATH))
    # random_points_fnames = n_random_subset(subset_size, train_image_paths)
    random_points_fnames = random.sample(train_image_paths, int(args.subset_size*len(train_image_paths)))
    print(len(random_points_fnames))
    utils = ModelUtils(filenames=random_points_fnames)
    print(colored("Creating Initial Dataset from Random Subset..",'blue'))
    utils.prepare_dataset(args.DIVERSE_DATA_PATH)
    
    da = DaTechniques(subset_size = args.subset_size, sample_size = args.sample_size)

    for cycle in range(args.cycles):  
      print(colored(f"Cycle {cycle+1} of {args.cycles}",'blue'))
      #Splitting Data into train and validation
      print(args.DATA_PATH,args.DIVERSE_DATA_PATH)
      if not (os.path.isdir(f"{args.DIVERSE_DATA_PATH}/train") and os.path.isdir(f"{args.DIVERSE_DATA_PATH}/val")) and args.val_split != 0 and args.VAL_PATH is None: 
          ...
          splitfolders.ratio(args.DIVERSE_DATA_PATH, output=f'./split_data_{log_name[:-5]}', ratio=(1-args.val_split-args.withhold_split, args.val_split, args.withhold_split), seed = args.seed)
          args.DIVERSE_DATA_PATH = f'./split_data_{log_name[:-5]}/train'
      ...
      args.model = f"./models/{log_name}"
      model = load_model(args)
      utils = ModelUtils(MODEL=model, DATA_PATH=args.DATA_PATH, img_size=args.image_size, embedding_size=args.embedding_size, embeddings=None, filenames=None)
      filenames, feature_list = utils.get_embeddings()
      print(len(filenames),len(feature_list))

      # import pdb;pdb.set_trace()
      # da = DaTechniques(subset_size = args.subset_size, filenames = filenames, embeddings = feature_list, sample_size = args.sample_size)
      da.filenames = filenames
      da.embeddings = feature_list
      if args.DA_TECHNIQUE=='DA_STD':
        print(colored("Running DA Standard..",'blue'))
        da_files, da_embeddings, _  = da.min_max_diverse_embeddings(i = da.farthest_point())
      elif args.DA_TECHNIQUE=='DA_FAST':
      ...
      print(colored("Creating Diverse Dataset from Diversity Algorithm..",'blue'))
      utils.prepare_dataset(args.DIVERSE_DATA_PATH)
